...
async def upload_document(
    ...
    if extension not in ALLOWED_TYPES:
        raise HTTPException(
            ...
            detail="Unsupported file type. Supported formats: PDF, DOC, DOCX, TXT, Markdown.",
        )
    ...
    try:
        supabase.storage.from_(BUCKET).upload(
            storage_path,
            data,
            {"content-type": file.content_type or ALLOWED_TYPES[extension], "upsert": "false"},
        )
    ...
    return {
        "document": document,
        "processing_job": job_result.data[0],
